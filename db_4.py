...
try:
    connection = psycopg2.connect(
        host="<localhost:5432>",  # имя хоста базы данных
...
with psycopg2.connect(database="postgres", user="postgres", password="postgres") as conn:
    ...
    add_client(conn, "John", "Doe", "johndoe@ex.com", "+1234567890")

    # Добавление клиента без указания телефона
    add_client(conn, "Jane", "Smith", "janesmith@ex.com")

    # Добавление телефона для существующего клиента
    add_phone(conn, client_id=1, phone="+79999999999")

    # Изменение данных о клиенте
    change_client(conn, client_id=1, email="johndoe_updated@ex.com")

    # Удаление телефона для существующего клиента
    delete_phone(conn, client_id=1, phone="+79999999999")
